...
export const useStoryGeneration = (
  ...
  const handleGenerateNextChapter = async (
    ...
    try {
      ...
      if (onGenerationSuccess) {
        onGenerationSuccess(
          maintenanceStory.model || '~deepseek/deepseek-flash-latest',
          actualCharge,
        );
      }
  ...
  const handleRegenerateChapter = async (
    ...
    try {
      ...
      if (onGenerationSuccess) {
        onGenerationSuccess(
          selectedStory.model || '~deepseek/deepseek-flash-latest',
          actualCharge,
        );
      }
  ...
  const handleAutoGenerateRemaining = async (): Promise<void> => {
    ...
    try {
      while ((activeStory.chapters?.length ?? 0) < activeStory.totalChapters) {
        ...
        if (onGenerationSuccess) {
          onGenerationSuccess(
            activeStory.model || '~deepseek/deepseek-flash-latest',
            batchCreditsCost,
          );
        }
