    ...
    <div className="lg:col-span-1 order-2 lg:order-1 bg-tj-bg-card p-4 sm:rounded-2xl rounded-none border-x-0 border-y sm:border border-tj-border-main sm:shadow-sm shadow-none space-y-4 h-fit">
      ...
      <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl">
        <button
          ...
          className={`flex-1 py-1.5 px-2 rounded-lg text-xs font-semibold transition-all cursor-pointer text-center ${
            sidebarTab === 'chapters'
              ? 'bg-white dark:bg-slate-700 shadow-xs text-tj-primary dark:text-tj-primary-hover font-bold'
        ...
        >
          ...
          <span>Notes</span>
          {highlights.length > 0 && (
            <span className="px-1.5 py-0.2 bg-tj-primary-light dark:bg-slate-600 text-tj-primary dark:text-tj-primary-hover text-[10px] rounded-full font-bold">
              {highlights.length}
            </span>
          )}
        </button>
      ...
      {sidebarTab === 'notes' ? (
        <StoryNotesSidebarTab
          ...
          isLoggedIn={Boolean(currentUser?.uid || currentUser?.id)}
        />
      ) : (
        <>
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
            ...
            {currentChaptersLoaded < totalChaptersCount ? (
              <div className="space-y-3">
                ...
                <button
                  disabled={
                    isLoadingNext || isAutoGeneratingRemaining || !isOnline
                  }
                ...
                >
                  {isAutoGeneratingRemaining ? (
                    <span className="flex items-center justify-center gap-1.5 text-tj-primary">
                      ...
                      {Math.min(currentChaptersLoaded + 5, totalChaptersCount)}{' '}
                      of {totalChaptersCount}
                    </span>
                  ) : (
                    <span>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-tj-text-muted mt-1 leading-relaxed">
                        A 10-chapter consistency check has been performed.
                        Please review the audit before resuming.
                      </p>
                    </div>
                  </div>
