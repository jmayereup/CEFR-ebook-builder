...
const cdnUrl = (process.env.VITE_COVER_CDN_URL || 'https://files.teacherjake.com').replace(/\/+$/, '');
...
async function main() {
  ...
  console.log(`Mode:           ${isReupload ? 'REUPLOAD / FIX' : 'DRY RUN (Use --reupload to upload missing covers)'}`);
  ...
  const stories = await pb.collection('stories').getFullList({ filter, sort: '-created' });
      ...
      batch.map(async (story: any) => {
        ...
        if (r2Status !== 'VALID') {
          if (localPath) {
            ...
            if (isReupload) {
              try {
                console.log(`[REUPLOAD] Uploading ${localPath} for story ${story.id} ("${story.title}")...`);
                const fileBuf = fs.readFileSync(localPath);
                const ext = path.extname(localPath).toLowerCase();
                const mimeType = ext === '.png' ? 'image/png' : 'image/jpeg';
                const formData = new FormData();
                formData.append('cover', new Blob([fileBuf], { type: mimeType }), `${story.id}${ext}`);

                const updated = await pb.collection('stories').update(story.id, formData);
                reuploadedCount++;
                ...
                console.error(`  -> Error reuploading for ${story.id}:`, uploadErr.message || uploadErr);
              }
            } else {
              actionTaken = `Local cover found (${path.basename(localPath)}). Ready for --reupload`;
            }
          } else {
            actionTaken = 'Missing on R2 & No local cover found. Needs cover generation.';
          }
        }
  ...
  console.log(`  - Local cover files missing:${missingR2Count - localFoundCount}`);
  if (isReupload) {
  ...
  if (missingOrBroken.length > 0) {
    console.log(`DETAILS FOR MISSING / BROKEN STORIES (${missingOrBroken.length}):`);
    console.log(`--------------------------------------------------`);
    for (const r of missingOrBroken) {
      ...
      console.log(`  Local Cover: ${r.localCoverPath ? r.localCoverPath : 'NOT FOUND'}`);
