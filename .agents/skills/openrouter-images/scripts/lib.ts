...
function reportHttpError(
  ...
  switch (status) {
    ...
    case 402:
      console.error(
        'Error 402: Insufficient credits. Add credits at https://openrouter.ai/credits',
      );
      break;
    case 404:
...
export async function getImageModelEndpoints(
  model: string,
): Promise<ImageModelEndpointsResponse> {
  const [author, slug] = model.split('/');
  if (!author || !slug) {
    console.error(
      `Error: Model must be in "author/slug" form (got "${model}").`,
    );
    process.exit(1);
  }
...
function parseProviderOptions(raw: string): Record<string, unknown> {
  ...
  try {
    ...
    console.error(
      'Error: --provider-options must be a JSON object keyed by provider slug.',
    );
    process.exit(1);
  }
