 ...
 */
import { fetchGenerationContent, parseArgs, requireApiKey } from './lib.js';
...
if (json) {
  ...
  if (!rawData) {
    ...
    console.log('This may be because Zero Data Retention (ZDR) was enabled.');
    console.log('');
  ...
  const hasInput = Boolean(
    input &&
      (input.prompt != null || (input.messages && input.messages.length > 0)),
  );
